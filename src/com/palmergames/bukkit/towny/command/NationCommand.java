...
public class NationCommand extends BaseCommand implements CommandExecutor {
	...
	static {
		...
		nation_help.add(ChatTools.formatCommand("", "/nation", "listcause [nation]", TownySettings.getLangString("nation_help_12")));
		...
		king_help.add(ChatTools.formatCommand(TownySettings.getLangString("king_sing"), "/nation", "call [nation]", TownySettings.getLangString("nation_help_11")));
		king_help.add(ChatTools.formatCommand(TownySettings.getLangString("king_sing"), "/nation", "declare [nation] [casus belli]", TownySettings.getLangString("nation_help_13")));
		king_help.add(ChatTools.formatCommand(TownySettings.getLangString("king_sing"), "/nation", "justify [nation] [casus belli]", TownySettings.getLangString("nation_help_14")));
		king_help.add(ChatTools.formatCommand(TownySettings.getLangString("king_sing"), "/nation", "surrender [nation]", TownySettings.getLangString("nation_help_15")));
		king_help.add(ChatTools.formatCommand(TownySettings.getLangString("king_sing"), "/nation", "cede [town] [nation]", TownySettings.getLangString("nation_help_16")));
...
//						throw new TownyException(TownySettings.getLangString("msg_err_command_disable"));
				...
				if (player.isOp()) {
					...
					if (split.length == 1)
						...
						if (!resident.isKing())
							throw new TownyException(TownySettings.getLangString("msg_err_for_kings_only"));
						nationInstantJustify(player, split[1], split[2]);
					}
				} else {
...
//						throw new TownyException(TownySettings.getLangString("msg_err_command_disable"));
				...
				if (enemyNation != null) {
					...
					TownyMessaging.sendMessage(player, String.format(TownySettings.getLangString("status_war_score"), war.getAttackerWarscore() * 100) + "%");
					TownyMessaging.sendMessage(player, String.format(TownySettings.getLangString("status_war_casualties"), war.getCasualtyCount(war.getAttacker())));
					...
					TownyMessaging.sendMessage(player, String.format(TownySettings.getLangString("status_war_score"), war.getDefenderWarscore() * 100) + "%");
					TownyMessaging.sendMessage(player, String.format(TownySettings.getLangString("status_war_casualties"), war.getCasualtyCount(war.getDefender())));
				...
				if (split.length == 1)
					...
					nationAddWargoal(player, split[1], split[2], params);
				}
...
//							townyUniverse.getDataSource().saveWar(war);
					...
					TownyMessaging.sendGlobalMessage(String.format(TownySettings.getLangString("msg_declared_independence"), townNation.getName(), finalNation.getName()));
...
//			TownyMessaging.sendPrefixedTownMessage(town, String.format(TownySettings.getLangString("msg_town_left_nation"), StringMgmt.remUnderscore(nation.getName())));
	...
	public void nationInstantJustify(Player player, String enemyNationName, String casusBelliName) throws TownyException, CloneNotSupportedException {
		...
		if (nation != null) {
			...
			Confirmation confirmation = new Confirmation(() -> {
				...
				universe.getDataSource().saveNation(playerNation);
				TownyMessaging.sendErrorMsg(player, String.format(TownySettings.getLangString("msg_justified_on"), playerNation.getName(), nation.getName(), finalCasusBelli.getName()));
			});
			ConfirmationHandler.sendConfirmation(player, confirmation);
		}
	...
	public void nationDeclare(Player player, String enemyNationName, String casusBelliName, String[] params) throws TownyException {
		...
		if (nation != null) {
			...
			if (playerNation.isAlliedWith(nation)) {
				throw new TownyException(TownySettings.getLangString("msg_err_allied_with"));
			}
			...
			Confirmation confirmation = new Confirmation(() -> {
				...
				TownyMessaging.sendGlobalMessage(String.format(TownySettings.getLangString("msg_declared_on"), playerNation.getName(), nation.getName(), finalCasusBelli.getName()));
			});
			ConfirmationHandler.sendConfirmation(player, confirmation);
	...
	public void nationAddWargoal(Player player, String enemyNationName, String casusBelliName, String[] params) throws TownyException, CloneNotSupportedException {
		com.palmergames.bukkit.towny.TownyUniverse universe = com.palmergames.bukkit.towny.TownyUniverse.getInstance();
		Nation nation;
		Nation remainingNation;
...
//			}
			...
			Confirmation confirmation = new Confirmation(() -> {
				...
				playerNation.setInfamy(playerInfamy + infamy);
				if (war.isAnAttacker(playerNation)) {
					war.addAttackerCasusBelli(finalCasusBelli);
				} else if (war.isADefender(playerNation)) {
					war.addDefenderCasusBelli(finalCasusBelli);
				}
				universe.getDataSource().saveWar(war);
				universe.getDataSource().saveCasusBelli(finalCasusBelli);
				TownyMessaging.sendErrorMsg(player, String.format(TownySettings.getLangString("msg_add_wargoal"), nation.getName(), finalCasusBelli.getName()));

				Resident king = nation.getKing();
				if (BukkitTools.isOnline(king.getName())) {
					TownyMessaging.sendErrorMsg(BukkitTools.getPlayer(king.getName()), String.format(TownySettings.getLangString("msg_enemy_add_wargoal"), playerNation.getName(), finalCasusBelli.getName()));
				}
			});
			ConfirmationHandler.sendConfirmation(player, confirmation);
	...
	public void nationPeace(Player player, String enemyNationName) throws TownyException, EmptyNationException {
		...
		try {
			enemyNation = universe.getDataSource().getNation(enemyNationName);
			remainingNation = universe.getDataSource().getResident(player.getName()).getTown().getNation();
		} catch (NotRegisteredException e) {
			throw new TownyException(String.format(TownySettings.getLangString("msg_err_invalid_name"), enemyNationName));
		}
...
//			}
	...
	public void nationSurrender(Player player, String enemyNationName) throws TownyException, EmptyNationException {
		com.palmergames.bukkit.towny.TownyUniverse universe = com.palmergames.bukkit.towny.TownyUniverse.getInstance();
		Nation enemyNation;
		Nation remainingNation;
		...
		if (enemyNation != null) {
			...
			if (!BukkitTools.isOnline(king.getName())) {
				...
				Confirmation confirmation = new Confirmation(() -> {
					TownyMessaging.sendErrorMsg(BukkitTools.getPlayer(king.getName()), String.format(TownySettings.getLangString("msg_offering_surrender"), playerNation.getName()));
					Confirmation enemyConfirmation = new Confirmation(() -> {
						try {
	...
	public void nationCallAlly(Player player, String allyNationName) throws TownyException, EmptyNationException {
		com.palmergames.bukkit.towny.TownyUniverse universe = com.palmergames.bukkit.towny.TownyUniverse.getInstance();
		Nation allyNation;
		Nation remainingNation;
		...
		try {
			allyNation = universe.getDataSource().getNation(allyNationName);
			remainingNation = universe.getDataSource().getResident(player.getName()).getTown().getNation();
		} catch (NotRegisteredException e) {
			throw new TownyException(String.format(TownySettings.getLangString("msg_err_invalid_name"), allyNationName));
		}
		if (remainingNation.getName().equalsIgnoreCase(allyNationName))
			throw new TownyException(String.format(TownySettings.getLangString("msg_err_invalid_name"), allyNationName));

		if (allyNation != null) {
			if (!playerNation.isAlliedWith(allyNation)) {
				throw new TownyException(TownySettings.getLangString("msg_err_not_ally"));
			}


			Resident king = allyNation.getKing();
			if (!BukkitTools.isOnline(king.getName())) {
				throw new TownyException(String.format(TownySettings.getLangString("msg_err_king_of_that_nation_is_not_online"), allyNation.getName(), king.getName()));
			} else {
				for (War war : playerNation.getWars()) {
					if (war.isWarLeader(playerNation) && !war.isInWar(allyNation)) {
